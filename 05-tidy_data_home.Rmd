...
bmi_dta %>% 
  filter(!is.na(gender)) %>% 
  ggplot(aes(x = bmi_t0, group = factor(gender), fill = factor(gender))) + 
...
bmi_dta %>% 
  filter(!is.na(gender)) %>% 
  ggplot(aes(x = bmi_t0, y = age_t0, colour = factor(gender))) + 
...
For example, both the This behaviour is important to be aware of, because it can lead to the same code working differently depending on the order in which packages are loaded
